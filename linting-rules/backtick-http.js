...
const shared = require("./backtick-keywords")
        ...
        params.tokens.forEach((token, index, tokens) => {
            if (token.children != null) {
                ...
                for (let word of words) {
                    let keyword = word.line.match(keywordsRegex);
                    if (tokens[index - 1].type != "heading_open") {
                        if (word.type != "code_inline") {
                            onError({
                                lineNumber: word.lineNumber,
                                detail: "HTTP keyword '" + keyword + "' must be backticked.",
                                range: rangeFromRegExp(word.line, keywordsRegex)
                            })
                        }
                    } else {
                        if (word.type === "code_inline") {
                            onError({
                                lineNumber: word.lineNumber,
                                detail: "HTTP keyword '" + keyword + "' in header must not be backticked.",
                                range: rangeFromRegExp(word.line, keywordsRegex)
                            })
                        }
                    }
